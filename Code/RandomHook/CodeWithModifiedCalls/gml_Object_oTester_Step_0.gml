...
if (state == 1)
{
    ...
    if (statetime > 60 && statetime < 260)
    {
        ...
        {
            starget_angle = (floor(tas_random_range(0, 3.9))) * 90
            sangle_timer = 120
    ...
    if (statetime >= change_time)
    {
        ...
        if (attacks == 1 || attacks == 3)
            next_attack = tas_choose(3, 4, 5, 6)
        if (attacks == 2)
            next_attack = 2
        attacks += 1
        if (attacks > 3)
...
if (state == 100)
{
    ...
    if (statetime == 200)
    {
        repeat (20)
        {
            expl = instance_create((x + (tas_random_range(-45, 45))), (y + (tas_random_range(-45, 45))), oFXAnimSpark)
