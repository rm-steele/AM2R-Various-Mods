...
if (state == STANDING || state == RUNNING)
{
    ...
    if (state == RUNNING && speedboost)
    {
        if ((facing == RIGHT && (kRight == 0 || kLeft > 0)) || (facing == LEFT && (kLeft == 0 || kRight > 0)))
        {
            ...
            if inwater
            {
                repeat (2 + floor(tas_random(4)))
                {
                    bubble = instance_create(x, y, oLBubble)
                    if instance_exists(bubble)
                    {
                        bubble.hspeed = tas_random_range(-1.5, 1.5)
                        bubble.vspeed = -0.1 - tas_random(0.5)
                        bubble.alarm[0] = 90 + tas_random(120)
                    }
                }
            }
        }
...
if platformCharacterIs(IN_AIR)
{
    ...
    if (state == JUMPING && statetime > 4 && position_meeting(x, (y + 8), oSolid) == 0 && justwalljumped == 0 && walljumping == 0 && metroid_drain == 0)
    {
        if (kLeft > 0 && kJump == 1 && kJumpPushedSteps == 0 && position_meeting((x + 8), (y - 16), oSolid) == 1)
        {
            ...
            if inwater
            {
                repeat (4 + floor(tas_random(4)))
                {
                    ...
                    if instance_exists(bubble)
                    {
                        bubble.hspeed = (-tas_random(2))
                        bubble.vspeed = -0.1 - tas_random(1)
                        bubble.alarm[0] = 90 + tas_random(120)
                    }
                }
            }
    ...
    if (kJump && kJumpPushedSteps == 0 && vjump == 1 && aimdirection != 6 && aimdirection != 7 && novjump == 0 && state != AIRBALL && aimlock == 0 && metroid_drain == 0)
    {
        vjump = 0
        yVel = 0
        novjump = 10
        ...
        jumpfwd = 1
        if (global.spacejump && ((inwater == 0 && waterfall == 0) || global.currentsuit == 2) && metroid_drain == 0)
        {
            state = JUMPING
            yVel = initialJumpAcc
            jumpfwd = 1
...
{
    state = JUMPING
    if ((kLeft == 0 && kRight == 0) || aimlock || metroid_drain > 0)
        vjump = 1
    else
        vjump = 0
    yAcc += initialJumpAcc
...
if (kJump == 0 || platformCharacterIs(ON_GROUND))
    hijump = 0
if (global.hijump && kJump && state == JUMPING && hijump && metroid_drain == 0)
{
...
if (kJump && kJumpPushedSteps == 0 && state == JUMPING && global.spacejump == 1 && novjump == 0 && aimlock == 0 && metroid_drain == 0 && ((inwater == 0 && waterfall == 0) || global.currentsuit == 2) && (vjump == 0 || (vjump == 1 && aimdirection != 6 && aimdirection != 7)))
{
...
chStepSpiderBall()
if ((walking == 1 || ((inwater || waterfall > 0) && global.currentsuit < 2) || turning) && metroid_drain == 0 && state == RUNNING)
    speedboost_steps = 0
        ...
        if inwater
        {
            repeat (4 + floor(tas_random(4)))
            {
                ...
                if instance_exists(bubble)
                {
                    bubble.hspeed = tas_random_range(-1.5, 1.5)
                    bubble.vspeed = -0.1 - tas_random(0.5)
                    bubble.alarm[0] = 120 + tas_random(120)
                }
            }
        }
    }
...
if (state == KNOCKBACK2_LAND)
{
    ...
    if (fxtimer == 0)
    {
        smk = instance_create((x - 6 + tas_random(12)), (y - 4), oFXAnimSpark)
        smk.image_speed = 0.5
        smk.additive = 0
        smk.sprite_index = sSmoke1
        smk.image_alpha = 0.6
...
if (state == A4EXPL)
{
    ...
    if (statetime > 180 && statetime < 400)
    {
        if (fxtimer == 0)
        {
            smk = instance_create((x - 8 + tas_random(16)), (y - 2), oFXAnimSpark)
            smk.image_speed = 0.5
            smk.additive = 0
            smk.sprite_index = sSmoke1
            smk.image_alpha = 0.6
...
if (((inwater == 1 || waterfall > 0) && global.currentsuit < 2) || metroid_drain > 0)
{
    if (state == SPIDERBALL)
    {
        state = BALL
...
if (state == BALL && dash == 0 && global.spiderball == 1 && moverobj == 0 && invincible == 0 && ((global.currentsuit < 2 && inwater == 0 && waterfall == 0) || global.currentsuit == 2) && statetime > 1 && metroid_drain == 0)
{
...
if (global.spiderball == 1 && sball == 0 && moverobj == 0 && invincible == 0 && ((global.currentsuit < 2 && inwater == 0 && waterfall == 0) || global.currentsuit == 2) && (state == AIRBALL || (state == SJEND && sjball == 1)) && statetime > 1 && metroid_drain == 0)
{
    if ((global.opspdstyle == 1 && kDown && kDownPushedSteps == 0) || (global.opspdstyle == 0 && ((kAim && kAimPushedSteps == 0) || (kAim2 && kAim2PushedSteps == 0))) || (global.opspdstyle == 2 && (kAim || kAim2)))
    {
        morphing = 0
...
if (state == GRABBEDGAMMA)
{
    ...
    canturn = 0
    if (global.currentsuit == 0)
        global.samushealth -= 0.8
    if (global.currentsuit == 1)
        global.samushealth -= 0.5
    if (global.currentsuit == 2)
        global.samushealth -= 0.3
...
if (global.watertype == 1 && inwater && global.currentsuit < 2)
{
    ...
    burning = 1
    if (global.currentsuit == 0)
        global.samushealth -= 0.1
    if (global.currentsuit == 1)
        global.samushealth -= 0.05
    if (global.samushealth <= 0)
    {
        with (oControl)
            event_user(1)
    ...
    if (inwater == 0)
    {
        if (abs(yVel) > 1)
        {
            ...
            repeat (8 + floor(tas_random(4)))
            {
                ...
                if instance_exists(bubble)
                {
                    bubble.hspeed = tas_random_range(-1.8, 1.8)
                    bubble.vspeed = 0.3 + tas_random(0.7)
                }
            }
            if (global.currentsuit < 2)
                yVel *= 0.5
...
if (inwater && tas_random(15) < 1 && fxtimer == 0)
{
...
if inwater
{
    if (state == JUMPING && vjump == 0)
    {
        ...
        {
            if (fxtimer == 0 && tas_random(4) < 1)
            {
                bubble = instance_create(x, (y - 16), oLBubble)
                if instance_exists(bubble)
                {
                    bubble.alarm[0] = 60 + tas_random(200)
                    bubble.direction = tas_random(360)
                    bubble.speed = 0.2
                }
            }
        }
        ...
        if (global.screwattack == 1)
        {
            if (fxtimer == 0)
            {
                ...
                if instance_exists(bubble)
                {
                    bubble.alarm[0] = 60 + tas_random(120)
                    bubble.direction = tas_random(360)
                    bubble.speed = 0.5
                }
            }
    ...
    if (state == SUPERJUMP && statetime == 0)
    {
        if (sjdir == 0)
        {
            repeat (10)
            {
                bubble = instance_create((x + (tas_random_range(-8, 8))), y, oLBubble)
                if instance_exists(bubble)
                {
                    bubble.alarm[0] = 60 + tas_random(60)
                    bubble.direction = 90
                    bubble.speed = 0.5 + tas_random(1.5)
                }
            }
        }
        if (sjdir == 1 || sjdir == 3)
        {
            repeat (10)
            {
                bubble = instance_create(x, (y - tas_random(16)), oLBubble)
                if instance_exists(bubble)
                {
                    bubble.alarm[0] = 60 + tas_random(60)
    ...
    if (state == BRAKING && statetime < 60)
    {
        if (fxtimer == 0)
        {
            bubble = instance_create((x + (tas_random_range(-4, 4))), y, oLBubble)
            if instance_exists(bubble)
                bubble.alarm[0] = 60 + tas_random(60)
        }
    }
...
if (footstep == 0)
{
    if (state == RUNNING)
    {
        if ((abs(image_index) >= 4 && abs(image_index) < 4.9) || (abs(image_index) >= 9 && abs(image_index) < 9.9))
        {
            ...
            if (inwater == 0 && waterfall == 0 && metroid_drain == 0)
                footstep = 5
            else
            {
                footstep = 10
...
if (sfx_isplaying(sndSBallLoop) && sball == 0)
    sfx_stop(sndSBallLoop)
if (sfx_isplaying(sndLavaLoop) && burning == 0)
    sfx_stop(sndLavaLoop)
if (sfx_isplaying(sndPlantLoop) && plantdrainfx == 0)
    sfx_stop(sndPlantLoop)
if (sfx_isplaying(sndDrainLoop) && metroid_drainfx == 0 && queen_drainfx == 0)
