...
if (state == 2)
{
    ...
    else
    {
        dir = 90 - facing * 90
        proj = instance_create((x + 12 * facing), (y - 65), oTorizoFireball)
        proj.direction = dir - 30 + tas_random(60)
        proj.speed = projspeed
        proj.gravity = 0
        if (global.difficulty >= 2)
        {
...
if (state == 3)
{
    ...
    proj.hspeed = (1.5 + tas_random(5)) * facing
    proj.vspeed = 2 - tas_random(4)
    if (statetime < 230)
        alarm[0] = 7
    if (global.difficulty >= 2)
    {
        proj.hspeed = (2 + tas_random(9)) * facing
        proj.vspeed = 4 - tas_random(8)
        if (statetime < 230)
            alarm[0] = 6
    }
}
