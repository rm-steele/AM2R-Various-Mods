if (tas_random(100) <= argument2)
{
    pickup = floor(tas_random(4))
    if (pickup == 0)
    {
        if (global.samushealth < global.maxhealth && instance_number(oHPickup) < 6)
    ...
    if (pickup == 1)
    {
        if (global.samushealth < global.maxhealth && instance_number(oHPickupBig) < 3)
