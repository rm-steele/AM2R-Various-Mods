...
if (state == 0)
{
    ...
    head_follow = 1
    xVel = 0
    if (point_distance(x, y, oCharacter.x, y) < 160)
        event_user(1)
...
if (state == 1)
{
    ...
    {
        msl = instance_nearest(x, y, oMissile)
        if ((msl.smissile > 0 || noswipe == 0) && swiping == 0)
        {
...
{
    if (roaring > 0)
        head_frame = anim_frame(sMZeta_Head_Roar, head_frame, 0.25)
    else
        head_frame = anim_frame(sMZeta_Head_Roar, head_frame, -0.25)
}
with (oCharacter)
